...
function formatTime(minutes, seconds) {
  const min = minutes < 10 ? `${minutes}`.padStart(2, '0') : minutes;
  const sec = seconds < 10 ? `${seconds}`.padStart(2, '0') : seconds;
  return `${min}:${sec}`;
}
...
function isPalindrome(str) {
  const filtredStr = str.replace(/[^A-Za-z0-9]/g, '');
  for (let i = 0; i <= filtredStr.length / 2; i += 1) {
    const tailLetters = filtredStr[filtredStr.length - (i + 1)].toLowerCase();
    const headletters = filtredStr[i].toLowerCase();

    if (tailLetters !== headletters) return false;
  }
  return true;
}
/**
  ...
  return str
    ...
    .map((word) => {
      return word
        .split('')
        .map((letter) =>
          letter.match(/[A-Z]/) ? letter.toLowerCase() : letter.toUpperCase()
        )
        .join('');
    })
    .join(' ');
